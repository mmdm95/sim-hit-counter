...
namespace Sim\HitCounter\Helpers;

use PDO;
use PDOStatement;
use Sim\HitCounter\Exceptions\BindValueException;
...
class DB
{
     ...
     * @throws IDBException
     */
    public function insert(string $table_name, array $values, array $raw_values = []): bool
    {
        $columns = array_keys($values);
        $values = array_values($values);

        $rawColumns = array_keys($raw_values);
        $rawValues = array_values($raw_values);
        ...
        $sql = "INSERT INTO {$this->quoteName($table_name)} (" .
            implode(', ', array_merge($columns, $rawColumns)) .
            ") VALUES (" .
            implode(', ', array_merge($namedParameters, $rawValues)) .
            ") ";
     ...
     * @throws IDBException
     */
    public function update(string $table_name, array $values, ?string $where = null, array $bind_values = [], array $raw_values = []): bool
    {
        $i = 1;
        $namedParameters = [];
        $bindValues = [];
